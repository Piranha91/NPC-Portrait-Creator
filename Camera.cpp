...
Camera::Camera(glm::vec3 target, float radius, float yaw, float pitch, float roll) :
    ...
    SetInitialState(glm::vec3(0.0f, 50.0f, 0.0f), DEFAULT_RADIUS, YAW, PITCH, 0.0f);
    Reset();
}
...
void Camera::SetInitialState(glm::vec3 target_worldSpace_yUp, float radiusFromTarget, float yaw, float pitch, float roll) {
...
void Camera::Reset() {
    ...
    std::cout << "  [Reset] Roll:          " << m_initialRoll << std::endl;
    std::cout << "---------------------------------" << std::endl;
...
void Camera::updateCameraVectors() {
    ...
    if (abs(Roll) > 0.01f) {
        glm::mat4 rollMatrix = glm::rotate(glm::mat4(1.0f), glm::radians(Roll), Front_worldSpace_yUp);
        Right_localSpace_yUp = glm::vec3(rollMatrix * glm::vec4(Right_localSpace_yUp, 0.0f));
        Up_localSpace_yUp = glm::vec3(rollMatrix * glm::vec4(Up_localSpace_yUp, 0.0f));
    }
}